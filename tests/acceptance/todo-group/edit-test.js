...
test('visiting /todo-groups/1/edit shows a form to edit a todo-group with id 1', function(assert) {
  const model = server.create('todo-group');
  ...
  andThen(function() {
    ...
    assert.equal(findWithAssert('.page-title').text().trim(),
      ...
      ' then the current todo-group title (verb). ' +
      'Current title (verb) is: ' + model.verb);
  });
});
...
test('visiting /todo-groups/1/edit shows an edit todo-group form that' +
  ...
  andThen(function() {
    assert.equal(findWithAssert('.title-input').val(), model.verb,
...
test('user can submit the form on /todo-groups/1/edit to update todo-group with id 1', function(assert) {
  ...
  andThen(function() {
    ...
    assert.equal(firstGroup.verb, 'Music',
